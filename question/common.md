...
**A：**AccountID、DataSourceID需要在CDP增长平台上新建数据源，或从已创建的数据源中获取, 如不清楚或无权限请联系项目经理；操作步骤参考[帮助文档](/docs/android/base/Introduce)。
...
iOS 和Android SDK：以每隔默认时间15秒的情况下向服务器发送事件包（最大50条事件）。<br/>
发送失败处理策略：<br/>
