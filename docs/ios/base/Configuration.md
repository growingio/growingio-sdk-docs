...
上报间隔，GrowingIO SDK 会先将事件存入数据库中，然后以每隔默认时间 15 秒的情况下向服务器发送事件包（最大 50 条事件）。
#### 5. **sessionInterval**
...
如果 APP 集成的是[**埋点 SDK**](/docs/ios/base#埋点sdk集成)，则需要您添加 Hybrid 依赖
...
![add Package NoIDFA](./../../../static/img/ios/add_package_hybrid.png)
