...
有时我们埋点需要大量业务属性，但需要每次调用时都进行传值，这给埋点工作带来了一定程度上的无用重复劳动。现在我们可以通过`setGeneralProps`来给所有的埋点事件加上通用属性，从而免去一些不必要的重复劳动。也可以利用该方法为所有的埋点事件进行动态设置通用属性。
...
如需删除某字段，设值指定字段为空（`''`或 `undefined` 或 `null`）即可，获取属性值时SDK会自动过滤通用属性的空值。
...
**<font color="#FC5F3A">注意：</font><br/>**
**1）SDK版本>=3.8.2支持。**<br/>
**2）定义的通用属性名依然需要在平台上进行预定义。**
**3）该方法可多次调用，已有相同属性名的值会被覆盖。**

### 10、清除埋点通用属性(clearGeneralProps)

SDK提供了一次性清除所有通用属性的方法，调用该方法会移除所有通用埋点属性。
